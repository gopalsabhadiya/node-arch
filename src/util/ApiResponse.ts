...
abstract class ApiResponse {
  ...
  protected prepare<T extends ApiResponse>(res: Response, response: T): Response {
    return res.status(this.status).json(ApiResponse.sanitize(ApiResponse.nullPropsRemover(response)));
  }
  ...
  private static nullPropsRemover(obj: any): any {
    return Object.fromEntries(
        Object.entries(obj)
            .filter(([_, v]) => v != null)
            .map(([k, v]) => [k, v === Object(v) ? ApiResponse.nullPropsRemover(v) : v])
    );
  }
}
